...
Architecture:
    [Utility line]              [UPS-backed line]
        ESP32 (beacon +            Mini PC (this daemon)
        watchdog)                  Tapo P110 (smart switch)
                                   3.5" HDD
        WiFi router on its own UPS (2 hr).

    The ESP32 on the utility line is the "beacon": when the grid is up it
    is reachable, when the grid dies it goes silent. (It replaced an
    earlier Zeb SP110 smart plug whose WiFi power-save dropped pings and
    caused false outages.) The same ESP32 also acts as the watchdog that
    powers the Tapo — and thus the Mini PC — back on when the grid returns.

Detection:
    Mini PC pings the beacon (ESP32) every 60s. If it is unreachable for
    5 min:
...
class Config:
    # Network targets
    # beacon_ip: the device on the utility line the daemon pings to infer
    # grid state. Currently the ESP32 at 192.168.1.32.
    beacon_ip: str = "192.168.1.32"
    ...
    ping_packet_count: int = 5  # Multiple packets to handle a beacon whose WiFi radio power-saves
...
async def is_real_outage(config: Config) -> bool:
    ...
    Beacon down + gateway up   → real power cut (beacon lost utility power)
    Beacon down + gateway down → network issue (don't trigger shutdown)
    """
    ...
    if not gateway_alive:
        log.warning(
            f"Beacon is down BUT gateway {config.gateway_ip} is also down → "
...
async def run(config: Config) -> None:
    ...
    log.info(
        f"Power daemon starting. Beacon={config.beacon_ip} "
    ...
    try:
        ...
        while True:
            alive = await ping(
                config.beacon_ip, config.ping_timeout_sec, config.ping_packet_count
            )

            if alive:
                if consecutive_failures > 0:
                    log.info(
                        f"Beacon back online after "
                ...
                log.warning(
                    f"Beacon offline {elapsed}s / {config.failure_threshold_sec}s"
                )
...
def load_config_from_env() -> Config:
    ...
    config = Config()
    string_fields = (
        "beacon_ip", "gateway_ip", "tapo_ip", "tapo_email", "tapo_password",
