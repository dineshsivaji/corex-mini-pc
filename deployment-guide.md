...
## Phase 7 — Wait-for-storage (Docker/HDD race fix)

### Why

After every reboot, Docker starts before the USB-SATA HDD has finished
spinning up and mounting at `/mnt/storage`. Containers that bind-mount
paths under it (Immich, Filebrowser) come up with empty mounts and
report "missing files" until manually restarted.

This phase installs a systemd oneshot service that blocks
`docker.service` until the mount is real. Strict dependency: if the HDD
never appears within 120s, Docker refuses to start (loud failure beats
silent data corruption).

### 7.1 Install the helper script

```bash
sudo install -m 755 -o root -g root \
  storage-wait/wait-for-mount.sh /usr/local/bin/wait-for-mount.sh
```
...
sudo install -m 644 -o root -g root \
  storage-wait/wait-for-storage.service \
  /etc/systemd/system/wait-for-storage.service
```

### 7.3 Install the docker drop-in

```bash
sudo mkdir -p /etc/systemd/system/docker.service.d
sudo install -m 644 -o root -g root \
  storage-wait/docker-wait-for-storage.conf \
  /etc/systemd/system/docker.service.d/wait-for-storage.conf
```

The drop-in approach means apt upgrades of `docker.io` won't clobber
the change.
...
systemd-analyze verify wait-for-storage.service        # no output = OK
```

### 7.6 Reboot test (the real one)

```bash
sudo reboot
```

After Mini PC comes back:

```bash
journalctl -b -u wait-for-storage.service
# Expect: "/mnt/storage ready after Xs" — typically 5-30s

systemctl status docker.service
# active (running), Started after wait-for-storage.service finished

# Confirm storage-dependent containers see real files:
docker exec immich-server ls /usr/src/app/upload | head -3
```

### 7.7 Optional — failure-injection test

Simulate a stuck HDD:
...
After boot, `wait-for-storage.service` should fail at 120s and
`docker.service` should NOT start. Restore:
...
### 7.8 Tuning the timeout

Default 120s is in the service file's `ExecStart`. Override:

```bash
sudo systemctl edit --full wait-for-storage.service
# Change "120" to your value, save
sudo systemctl daemon-reload
```
...
| Containers show empty/missing files after reboot | HDD not yet mounted when Docker started — Phase 7 not deployed | Apply Phase 7, reboot, verify |
| Boot hangs for 2 min after reboot, then no Docker | `wait-for-storage` timed out — HDD didn't appear | Check `journalctl -b -u wait-for-storage`, fstab UUID, USB cable |
| `wait-for-storage` always times out | fstab UUID wrong, or HDD truly dead | Verify with `lsblk`, `blkid`; rollback Phase 7 if HDD failed |
